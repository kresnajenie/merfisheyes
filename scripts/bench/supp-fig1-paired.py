...
rows_all = max(len(v) for v in data.values())
fig = plt.figure(figsize=(17.0, 0.40 * rows_all + 1.8))
gs = fig.add_gridspec(1, 4, wspace=1.30,
                      left=0.135, right=0.99, top=0.88, bottom=0.10)
cells = {"a": gs[0, 0], "b": gs[0, 1], "c": gs[0, 2], "d": gs[0, 3]}
cellmax = {k: rows_all for k in cells}

for ltr, title, _ in PANELS:
    ax = fig.add_subplot(cells[ltr])
    entries = data[ltr]
    n, N = len(entries), cellmax[ltr]
    xmax = max([e["st"] or 0 for e in entries] + [e["bt"] or 0 for e in entries]) / 60
    for i, e in enumerate(entries):
        y = N - 1 - i  # top-aligned
        if e["bt"] is not None:
            ax.barh(y + 0.20, e["bt"] / 60, height=0.36, color=BLUE)
            ax.text(e["bt"] / 60 + xmax * 0.015, y + 0.20, fmt_t(e["bt"]), va="center", fontsize=5.4, color=BLUE)
        elif e["bnote"]:
            ax.text(xmax * 0.015, y + 0.20, e["bnote"], va="center", fontsize=5.2, color=GREY, style="italic")
        if e["st"] is not None:
            ax.barh(y - 0.20, e["st"] / 60, height=0.36, color=ORANGE)
            ax.text(e["st"] / 60 + xmax * 0.015, y - 0.20, fmt_t(e["st"], e["tier"]), va="center", fontsize=5.4, color=ORANGE)
    ax.set_yticks([N - 1 - i for i in range(n)], [e["label"] for e in entries])
    ax.set_ylim(-0.7, N - 0.3)  # equal row pitch across panels; blank below short panels
    ax.set_xlim(0, xmax * 1.40)
    ax.set_xlabel("processing time (min)", fontsize=6.5)
    ax.set_title(title, fontsize=7.5, loc="left", pad=4)
    ax.tick_params(length=2)
    ax.text(-0.42, 1.0 + 0.5 / max(n, 1), ltr, transform=ax.transAxes, fontsize=11, fontweight="bold")
...
for ext in ("svg", "pdf", "png"):
    fig.savefig(f"{SC}/supp_fig1_paired.{ext}", dpi=300)
print("saved supp_fig1_paired.{svg,pdf,png}")
